...
router.get("/themes", async (req, res) => {
    ...
    const list = await prisma_1.prisma.theme.findMany({
        where: { branchId },
        orderBy: { id: "asc" },
    });
    res.json(list.map((t) => ({
...
router.get("/admin/design/get-available-design-elements", async (_req, res) => {
    const elements = await prisma_1.prisma.designElementCatalog.findMany({
        orderBy: { id: "asc" },
    });
...
/* ---------------------------- THEME (current) ---------------------------- */
async function getCurrentTheme(branchId) {
    return prisma_1.prisma.theme.findFirst({ where: { branchId, isCurrent: true } });
}
router.get("/get-theme", async (req, res) => {
    const branchId = Number(req.query.branch_id || 1);
    const theme = await getCurrentTheme(branchId);
    if (!theme)
        return res.status(404).json({ error: "No current theme" });
    ...
    res.json({
        ...
        design_elements: await Promise.all(blocks.map(async (b) => {
            const catalog = b.designElementId
                ? await prisma_1.prisma.designElementCatalog.findUnique({
                    where: { id: b.designElementId },
                    select: { settingsSchema: true },
                })
                : await prisma_1.prisma.designElementCatalog.findFirst({
                    where: { code: b.code },
                    select: { settingsSchema: true },
                });
            return {
...
router.get("/get-theme-single-design-element/:id", async (req, res) => {
    ...
    const b = await prisma_1.prisma.themeDesignElement.findUnique({ where: { id } });
    if (!b)
    ...
    res.json({
        properties: {
            ...
            name: b.name,
            custom_name: b.customName,
            branch_id: (await prisma_1.prisma.theme.findUnique({ where: { id: b.themeId } }))
                ?.branchId ?? null,
            code: b.code,
...
router.post("/switch-theme", async (req, res) => {
    ...
    await prisma_1.prisma.theme.updateMany({ where: { branchId }, data: { isCurrent: false } });
    await prisma_1.prisma.theme.update({ where: { id: themeId }, data: { isCurrent: true } });
    res.json({ ok: true });
});
/* ------------------------------ BLOCKS CRUD ------------------------------ */
/**
...
router.post("/blocks", async (req, res) => {
    const { theme_id, design_element_id, name, custom_name, code, position, settings, is_active, } = req.body;
    // determine schema (by design_element_id or code)
    let schema = null;
    if (design_element_id) {
        const cat = await prisma_1.prisma.designElementCatalog.findUnique({
        ...
        const cat = await prisma_1.prisma.designElementCatalog.findFirst({
    ...
    if (schema) {
        const { valid, errors } = (0, validate_1.validateSettings)(schema, finalSettings);
        if (!valid)
            return res.status(400).json({ error: "Validation failed", details: errors });
    }
    const block = await prisma_1.prisma.themeDesignElement.create({
        data: {
...
router.patch("/blocks/:id", async (req, res) => {
    ...
    const existing = await prisma_1.prisma.themeDesignElement.findUnique({ where: { id } });
    if (!existing)
        return res.status(404).json({ error: "Not found" });
    let dataToUpdate = {};
    if (name !== undefined)
    ...
    if (custom_name !== undefined)
        dataToUpdate.customName = custom_name;
    if (position !== undefined)
        dataToUpdate.position = Number(position);
    if (typeof is_active === "number" || typeof is_active === "boolean") {
        dataToUpdate.isActive = !!is_active;
    }
    if (settings !== undefined) {
        const schema = await (0, catalog_1.getSchemaForThemeDesignElement)(id);
        if (schema) {
            const { valid, errors } = (0, validate_1.validateSettings)(schema, settings);
            if (!valid)
                return res.status(400).json({ error: "Validation failed", details: errors });
        }
        dataToUpdate.settings = settings;
    }
    const updated = await prisma_1.prisma.themeDesignElement.update({
        where: { id },
        data: dataToUpdate,
    });
...
router.delete("/blocks/:id", async (req, res) => {
    const id = Number(req.params.id);
    await prisma_1.prisma.themeDesignElement.delete({ where: { id } });
    res.json({ ok: true });
});
/* -------------------------------- PUBLISH -------------------------------- */
router.post("/publish", async (req, res) => {
    ...
    const snapshot = await prisma_1.prisma.themeSnapshot.create({
