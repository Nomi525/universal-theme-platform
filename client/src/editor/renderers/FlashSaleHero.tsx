// FlashSaleHero.tsx — compact, square corners + full-bleed bg image (fixed z-index)
...
export default function FlashSaleHero({ settings }: Props) {
  const {
    enabled = true,
  ...
  // Always have a URL so preview never looks blank
  const bgUrl =
  ...
  return (
    <section className="w-full">
      ...
      >
        {/* Background image + overlay (z-0 so they sit ABOVE the section bg) */}
        <img
          src={bgUrl}
          alt=""
          aria-hidden="true"
      ...
      {/* OPTIONAL SECTIONS (unchanged structurally) */}
      {show_deals_grid && (
        <div
          className="w-full"
          style={{
